xmlport 81600 "wanaBank Import CFONB120"
{
    ...
    trigger OnBeforeInsertRecord()var lRetour: Integer;
    begin
        CASE _EntryCode OF '01': StartingBalance;
        '04': Operation;
        '05': Details;
        '07': EndingBalance;
        END;
    ...
    local procedure GetBankAccount(): Boolean begin
        IF(_BankBranchNo = COPYSTR(DELCHR(BankAccount.IBAN), 10, 5)) AND (_BankAccountNo = COPYSTR(DELCHR(BankAccount.IBAN), 15, 11))THEN EXIT;
        IF BankAccount.FINDSET THEN REPEAT UNTIL(BankAccount.NEXT = 0) OR (_BankBranchNo = COPYSTR(DELCHR(BankAccount.IBAN), 10, 5)) AND (_BankAccountNo = COPYSTR(DELCHR(BankAccount.IBAN), 15, 11));
        if(_BankBranchNo <> COPYSTR(DELCHR(BankAccount.IBAN), 10, 5)) OR (_BankAccountNo <> COPYSTR(DELCHR(BankAccount.IBAN), 15, 11))THEN clear(BankAccount)
        else IF _CurrencyCode <> GeneralLedgerSetup."LCY Code" THEN BankAccount.TESTFIELD("Currency Code", _CurrencyCode);
    end;
    local procedure StartingBalance()begin
        GetBankAccount;
        IF(BankAccount."No." = BankAccReconciliation."Bank Account No.") or (BankAccount."No." = '')THEN EXIT;
    ...
    local procedure Operation()begin
        IF BankAccount."No." = '' THEN EXIT;
        ...
        IF _DocumentNo <> '0000000' THEN BankAccReconciliationLine."Check No.":=_DocumentNo;
    ...
    local procedure Details()var DataExchField: Record 1221;
    begin
        IF BankAccount."No." = '' THEN EXIT;
        DetailLineNo+=1;
        DataExchField.InsertRec(DataExch."Entry No.", LineNo, DetailLineNo, _Description, '');
    end;
    local procedure EndingBalance()begin
        IF BankAccount."No." = '' THEN EXIT;
    ...
    pNoOfDecimals: Text)ReturnValue: Decimal begin
        EVALUATE(ReturnValue, CONVERTSTR(pTextAmount, '{ABCDEFGHIJKLMNOPQR}', '01234567891234567890'));
        IF pTextAmount[14]IN['J' .. 'R', '}']THEN ReturnValue*=-1;
        CASE _NoOfDecimals OF '0': EXIT(ReturnValue);
        '1': EXIT(ReturnValue / 10);
        '2': EXIT(ReturnValue / 100);
        '3': EXIT(ReturnValue / 1000);
        END;
    end;
}
