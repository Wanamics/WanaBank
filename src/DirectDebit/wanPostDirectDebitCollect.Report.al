report 87411 "wan Post Direct Debit Collect."
// copy from report 1201 "Post Direct Debit Collection"
{
    ...
    dataset
    {
        dataitem("Direct Debit Collection Entry"; "Direct Debit Collection Entry")
        {
            ...
            */
            DataItemTableView = sorting("Direct Debit Collection No.", "Transfer Date", "Currency Code", "Entry No.");
    ...
    var
        ...
        //[
        wanBankGenJnlLine: Record "Gen. Journal Line";
    //]
    ...
    */
    ...
    local procedure wanBeforeCreateJnlLine();
    begin
        if ("Direct Debit Collection Entry"."Transfer Date" <> wanBankGenJnlLine."Posting Date") or
            ("Direct Debit Collection Entry"."Currency Code" <> wanBankGenJnlLine."Currency Code") then
            wanPostBankJnlLine();
    end;

    local procedure wanAfterCreateJnlLine();
    begin
        wanBankGenJnlLine."Posting Date" := GenJnlLine."Posting Date";
        wanBankGenJnlLine."Document No." := GenJnlLine."Document No.";
        wanBankGenJnlLine."Currency Code" := GenJnlLine."Currency Code";
        wanBankGenJnlLine.Amount -= GenJnlLine.Amount;
    end;

    local procedure wanPostDataItem()
    begin
        wanPostBankJnlLine();
    end;

    local procedure wanPostBankJnlLine()
    var
        DescriptionLbl: Label 'Direct Debit Sum';
    begin
        if wanBankGenJnlLine.Amount = 0 then
            exit;
        LastLineNo += 10000;
        wanBankGenJnlLine."Line No." := LastLineNo;
        wanBankGenJnlLine.Validate("Document Type", wanBankGenJnlLine."Document Type"::Payment);
        wanBankGenJnlLine.Validate("Account Type", wanBankGenJnlLine."Account Type"::"Bank Account");
        wanBankGenJnlLine.Validate("Account No.", DirectDebitCollection."To Bank Account No.");
        wanBankGenJnlLine.Description := DescriptionLbl;

        wanBankGenJnlLine.Validate("Posting Date");
        wanBankGenJnlLine.Validate("Currency Code");
        wanBankGenJnlLine.Validate(Amount);
        if CreateJnlOnly then
            wanBankGenJnlLine.Insert(true)
        else
            GenJnlPostLine.RunWithCheck(wanBankGenJnlLine);
        wanBankGenJnlLine.Amount := 0;
    end;
    //]
}
